...
class PostMongoAPIView:
    ...
    def single_post_request(self, request, *args, **kwargs):
        ...
        serializer_class = getattr(self, 'serializer_class', None)
        if not serializer_class:
            response_data = {'message': 'serializer class didnt set'}
            self.store_logs(requests=request, response=response_data, response_status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)
        ...
        serializer = serializer_class.get('POST', None)
        if not serializer:
            response_data = {'message': 'serializer class for post method didnt set'}
            self.store_logs(requests=request, response=response_data, response_status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)
            return JsonResponse(data=response_data,
                                status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        ...
        self.store_logs(requests=request, response=response_data,
